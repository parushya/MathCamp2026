...
Day 2 now ends after the Bayes' Quick Drill. Combinatorics and the
Random-Variables-through-Expectation-Algebra chain that used to follow it
have moved to 03-calculus-draft.qmd (retitled "Probability + Calculus"),
so that Day 2 stays focused on events/sets/axioms/Bayes and Day 3 runs the
RV/expectation chain back-to-back with limits/differentiation/integration
without an unrelated topic switch in between.
-->
...
(function () {
  ...
  function draw() {
    ...
    var lower = Math.max(0, pa + pb - 1);
    var upper = Math.min(pa, pb);
    var pab = lower + ov * (upper - lower);
    var pau = pa + pb - pab;
    var sum = pa + pb;
