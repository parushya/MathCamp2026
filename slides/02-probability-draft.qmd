...
::: {.incremental}
-   Everything so far has been about **events** — subsets of $\Omega$, yes/no questions: is the country a democracy, is growth positive. Events can be combined (union, intersection) but not added or averaged.
-   A **random variable** assigns a *number* to every outcome: $X: \Omega \to \mathbb{R}$. Once outcomes are numbers, ordinary arithmetic applies — sums, averages, variances.
-   This is the switch the rest of today runs on: "is the event {Democracy} likely?" becomes "what's the expected value of the underlying democracy score itself?"
:::

```{=html}
<div class="running-example-note">&#8618; Running example: the *event* "Democracy" = {democracy score &gt; 0.5} is a yes/no subset of &Omega;; the *random variable* "democracy score" assigns each of the 174 countries its own number in [0,1] — now we can average it, take its variance, condition on it.</div>
```
