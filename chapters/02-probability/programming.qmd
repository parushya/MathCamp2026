...
Learning to code therefore requires two skills:
...
Notice that neither of these skills is really about programming. Coding is simply the tool that translates your analytical thinking into a sequence of instructions that the computer can execute.
...
2.    **Lists**: Lists are the most flexible form of data structures and can hold objects with different datatypes (i.e., heterogeneous).
...
4. Try to extract the first, second, fifth, and tenth columns of this dataset and save them into a new object. What is this new dataset about? Do you think it contains information that is helpful?

5. Can you find a way to extract rows only in which the exporting state is the United States?
...
```{r eval=FALSE}
new <- JPRdata_select[, c(1, 2, 5, 9)]

## arguably a better way

new_updated <- JPRdata_select[, c(1, 2, 3, 5, 6, 9)]
```

#### Question 5

```{r eval=FALSE}
new_US <- new_updated[new_updated$ccode_e==2,]
dim(new_US)
